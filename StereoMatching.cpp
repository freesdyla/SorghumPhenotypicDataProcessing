...
bool StereoMatching::BMStereo(std::vector<cv::Mat>& stereoPair, cv::Mat& disparity)
{
	if (!checkStereoPairValid(stereoPair))
		return false;
	int numDisp = 16 * 4;
	...
	cv::Mat disp, disp8;
  	bm->compute(stereoPair[0], stereoPair[1], disp);
...
bool StereoMatching::SGBMStereo(std::vector<cv::Mat>& stereoPair, int numDisp_x16, bool display, cv::Mat& disparity)
{
	
	int numDisp = 16 * numDisp_x16;
	int blockSize = 3;

	cv::Ptr<cv::StereoSGBM> sgbm = cv::StereoSGBM::create(0, numDisp, blockSize);

	sgbm->setPreFilterCap(63);

//	sgbm->setBlockSize(blockSize);

	int cn = stereoPair[0].channels();
	...
	sgbm->compute(stereoPair[0], stereoPair[1], disp);
